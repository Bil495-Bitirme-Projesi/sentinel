...
class _CameraFormScreenState extends State<CameraFormScreen> {
  ...
  Future<void> _submitForm() async {
    ...
    // Kullanıcı geçersiz bir metin girerse varsayılan olarak 80.0 alalım
    final thresholdVal = double.tryParse(_thresholdController.text.trim()) ?? 80.0;

    try {
      if (_parsedId == null) {
        // Yeni Kamera Oluştur
        await CameraService.instance.createCamera(
          _nameController.text.trim(),
          _rtspUrlController.text.trim(),
          thresholdVal, // YENİ PARAMETRE
        );
      } else {
        // Mevcut Kamerayı Güncelle
        await CameraService.instance.updateCamera(
          _parsedId!,
          _nameController.text.trim(),
          _rtspUrlController.text.trim(),
          thresholdVal, // YENİ PARAMETRE
      ...
      if (e is DioException && e.response != null) {
        errorMessage = 'Backend Hatası: ${e.response?.data}';
      } else {
        errorMessage = 'Hata: $e';
      }
